...
namespace SMProject.Common
{
    class USBPrint
    {
        ...
        public static void Print(System.Drawing.Printing.PrintPageEventArgs e, List<SalesListDetail> list,
            string serialNumber, string salesperson)
        {
            ...
            for (int i = 1; i <= list.Count; i++)
            {
                ...
                totalMoney += list[i - 1].SubTotalMoney;
                decimal unitPrice = list[i - 1].UnitPrice * Convert.ToDecimal(1.00);
                if (list[i - 1].Discount != 0)
                ...
                e.Graphics.DrawString(list[i - 1].ProductFullName, font, Brushes.Blue, left, nextTop, new StringFormat());
